    ...
    if G_DRIVE_DATA is not None:
        try:
            ...
            await gdrive.edit(
                "`[AUTHENTICATE - ERROR]`\n\n"
                "`Status` : **BURUK**\n"
                "`Alasan` : Entitas **G_DRIVE_DATA** tidak valid!"
            )
            return False
    else:
        """ - Only for old user - """
...
async def download_gdrive(gdrive, service, uri):
    ...
    try:
            ...
            try:
                ...
                try:
                    ...
                    try:
                        ...
                        reply += (
                            "`[FILE - ERROR]`\n\n"
                            "`Status` : **BURUK** - gagal untuk mengunduh.\n"
                            "`Alasan` : uncaught err."
                        )
                    else:
                        reply += (
        ...
        if mimeType == "application/vnd.google-apps.folder":
            await gdrive.edit("`Membatalkan, folder unduhan tidak didukung...`")
            return False
        file_path = TEMP_DOWNLOAD_DIRECTORY + file_name
...
async def lists(gdrive):
    ...
    message = ""
    fields = (
        "nextPageToken, files(name, id, " "mimeType, webViewLink, webContentLink)"
    )
    page_token = None
    result = []
    while True:
    ...
    if len(message) > 4096:
        await gdrive.edit("`Hasilnya terlalu besar, dikirim sebagai file...`")
    ...
    if isfile(value):
        ...
        if not uri and not gdrive.reply_to_msg_id:
            await gdrive.edit(
                ...
                "nilai file/folder, cth: `.gd <namafile1> <namafile2>` "
